...
Sudarshan et al.~\cite{sudarshan2025object} demuestran que los ViTs superan a las CNNs en precisión cuando se dispone de grandes volúmenes de datos de entrenamiento, alcanzando un \textit{accuracy} del 93\,\% en tareas de clasificación general de objetos. Además, los transformers preentrenados en grandes corpus pueden transferir conocimiento de forma más efectiva a tareas específicas, una característica de valor crítico para aplicaciones industriales donde los datos etiquetados son escasos. Con estos fundamentos establecidos, se revisan a continuación las técnicas de aprendizaje auto-supervisado que permiten aprovechar los ViT en escenarios con datos limitados.
...
La validación experimental en tres datasets industriales distintos confirma la efectividad de combinar localidad (CNN) y globalidad (Transformer). Esta línea de trabajo sugiere que, dependiendo de las características específicas del problema de detección, las arquitecturas híbridas pueden ofrecer ventajas sobre aproximaciones puras. Independientemente de la elección arquitectural, la escasez de datos etiquetados constituye un desafío transversal; la siguiente sección analiza cómo el transfer learning y el aprendizaje few-shot permiten abordar esta limitación.

\section{Transfer learning y aprendizaje con datos limitados}

La escasez de datos etiquetados constituye uno de los principales obstáculos para la aplicación industrial de sistemas de detección de anomalías. Esta sección analiza cómo el transfer learning y el aprendizaje few-shot permiten abordar esta limitación, presentando trabajos que validan la efectividad de Vision Transformers preentrenados en entornos industriales reales, así como evaluaciones sistemáticas que orientan la selección de arquitecturas bajo restricciones prácticas.
...
Aunque Alber et al.~\cite{alber2024evaluating} proporcionan una evaluación comprehensiva de modelos ViT combinados con algoritmos de detección de anomalías, su trabajo se centra en configuraciones específicas. Existe oportunidad para un benchmarking más exhaustivo que compare sistemáticamente: diferentes variantes de ViT preentrenado (DINOv2, DINOv3, ViT-L, modelos más pequeños para edge deployment), estrategias de fine-tuning (full fine-tuning vs. parameter-efficient methods), y métodos de detección de anomalías (reconstruction-based, memory-bank, distribution-based).
...
\begin{enumerate}
    ...
    \item Benchmarking sistemático que compara arquitecturas ViT con métodos convolucionales tradicionales bajo condiciones realistas de datos limitados.
