...
La arquitectura Vision Transformer, introducida originalmente por Dosovitskiy et al., representa un cambio paradigmático en el procesamiento de imágenes. Mientras que las CNNs procesan información mediante operaciones de convolución local, los ViTs dividen la imagen en parches y los procesan mediante mecanismos de self-attention que capturan relaciones entre todas las regiones de forma simultánea~\cite{sudarshan2025object}. Esta diferencia arquitectural confiere a los transformers ventajas significativas en términos de escalabilidad, flexibilidad en el manejo de imágenes de diferentes resoluciones, y capacidad de transfer learning.
...
Una de las limitaciones más significativas de los modelos de aprendizaje profundo en entornos industriales es su dependencia de grandes cantidades de datos etiquetados manualmente. Khan et al.~\cite{khan2024survey} presentan una revisión exhaustiva de 42 páginas sobre mecanismos de aprendizaje auto-supervisado (Self-Supervised Learning, SSL) específicamente diseñados para Vision Transformers, identificando esta problemática como uno de los desafíos centrales del campo.

El aprendizaje auto-supervisado se fundamenta en la explotación de las relaciones inherentes dentro de los datos, utilizándolas como forma de supervisión sin requerir anotaciones humanas. Khan et al.~\cite{khan2024survey} desarrollan una taxonomía completa de técnicas SSL aplicables a ViTs, clasificándolas según sus representaciones y tareas de pre-entrenamiento. Entre las metodologías más relevantes identificadas se encuentran:

\textbf{Contrastive Learning:} Métodos como SimCLR y MoCo adaptados a arquitecturas transformer, que aprenden representaciones discriminativas mediante la comparación de vistas aumentadas de la misma imagen.

\textbf{Masked Image Modeling:} Inspirado en BERT del procesamiento de lenguaje natural, donde regiones de la imagen se enmascaran y el modelo aprende a predecirlas. Masked Autoencoders (MAE) han demostrado ser particularmente efectivos para ViTs.

\textbf{Self-distillation:} Aproximaciones como DINO y DINOv2, donde el modelo aprende de sus propias predicciones sin etiquetas, generando representaciones visuales robustas.
