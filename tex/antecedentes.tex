...
A diferencia de las \acrshort{CNN}, que procesan información local mediante filtros convolucionales y deben apilar muchas capas para ampliar el campo receptivo, los \acrshort{ViT} obtienen un campo receptivo global desde la primera capa de atención. Esta característica confiere ventajas fundamentales: (1) capacidad de modelar dependencias a largo alcance entre regiones distantes de la imagen, (2) escalabilidad favorable —el rendimiento mejora consistentemente con el tamaño del modelo y del corpus de preentrenamiento—, y (3) mayor transferibilidad a nuevos dominios. La tabla~\ref{tab:cnn-vit-comparativa} resume las diferencias arquitecturales clave entre ambos paradigmas.
...
\caption{Arquitectura Vision Transformer (ViT). La imagen de entrada se divide en parches de tamaño fijo ($16\times16$~px) que se proyectan linealmente a un espacio de dimensión $D$. Se añade un token de clase especial \texttt{[CLS]} y embeddings de posición para preservar la información espacial. La secuencia resultante se procesa mediante bloques Transformer Encoder con mecanismo de Multi-Head Self-Attention, obteniendo un campo receptivo global desde la primera capa. La representación del token \texttt{[CLS]} en la última capa se usa para clasificación; las representaciones de parches permiten tareas de detección y segmentación.}
...
\caption{Logotipo de DINOv3, modelo de aprendizaje auto-supervisado que constituye el backbone del sistema de detección desarrollado en este trabajo.}
...
DEIMv2 sigue el paradigma RT-DETR, compuesto por tres módulos principales: (1) un backbone ViT con DINOv3, (2) un \acrfull{STA} para adaptación de características multi-escala, y (3) un decodificador basado en atención cruzada para refinamiento iterativo de predicciones. La cadena de procesamiento es la siguiente: la imagen de entrada se divide en parches de $16\times16$ píxeles que el backbone ViT procesa mediante \textit{Multi-Head Self-Attention}; las salidas de tres capas intermedias (5ª, 8ª y 11ª del ViT) se extraen para construir una representación multi-escala. La Figura~\ref{fig:diagrama-deimv2} ilustra esta arquitectura de forma esquemática.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{diagramas-arquitectura/diagrama-deimv2.png}
\caption{Arquitectura DEIMv2. El pipeline integra: (1) backbone ViT preentrenado con DINOv3, que extrae características globales con campo receptivo completo; (2) \acrshort{STA}, que combina mediante fusión Bi-Fusion un ramal semántico (interpolación de características ViT) y un ramal de detalle fino (CNN ultraligera) para producir representaciones multi-escala a $1/8$, $1/16$ y $1/32$ del tamaño de entrada; y (3) decodificador DETR basado en atención cruzada que refina iterativamente un conjunto de consultas de objetos para generar las detecciones finales sin post-procesado NMS.}
...
La elección de DEIMv2 como arquitectura principal para el presente TFG se fundamenta en tres razones concretas: (1) la integración nativa de las representaciones auto-supervisadas de DINOv3, que han demostrado superior transferibilidad a dominios industriales~\cite{huang2025semiconductor}; (2) la capacidad de detección multi-escala sin fine-tuning completo del backbone; y (3) el paradigma DETR que elimina la necesidad de \acrshort{RPN} y procedimientos de supresión de no-máximos (\textit{NMS}), simplificando el pipeline y reduciendo errores de post-procesado.
...
El trabajo propuesto en este \gls{TFG} busca contribuir a cerrar estas brechas mediante: (1) aplicación de Vision Transformers preentrenados con DINOv3~\cite{simeoni2025dinov3} a detección de anomalías en componentes electrónicos diversos (PCBs, conectores, SMD), (2) benchmarking sistemático comparando arquitecturas ViT con métodos tradicionales bajo condiciones de datos limitados realistas, (3) validación práctica con componentes reales capturados en condiciones variables, y (4) desarrollo de un sistema de demostración que integre detección con visualización de mapas de atención para interpretabilidad.
