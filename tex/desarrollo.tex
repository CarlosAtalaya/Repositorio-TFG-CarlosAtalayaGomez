...
La Etapa 3 aplica un análisis de calidad sobre el dataset curado inicial, orientado a:
\begin{itemize}
...
La Etapa 4 realiza la preparación final mediante cuatro subetapas (4.1 a 4.4) que transforman secuencialmente el dataset:
\begin{itemize}
...
La Etapa 5 cierra el pipeline validando la consistencia del dataset final mediante:
\begin{itemize}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Separación estricta entre conjuntos de entrenamiento, validación y test.
        \item Uso del conjunto de test únicamente para evaluación final.
        \item Selección del mejor checkpoint basada en métricas de validación.
...
La descripción conceptual de ResNet-18 y EfficientNet-B0 se presenta en la sección~\ref{subsec:cnn-arquitecturas-referencia} de los Antecedentes. Las Figuras~\ref{fig:diagrama-resnet18} y~\ref{fig:diagrama-efficientnet} muestran sus arquitecturas.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{diagramas-arquitectura/resnet18-arquitetcura-diagrama.png}
\caption{Arquitectura ResNet-18~\cite{he2016resnet}.}
...
\caption{Arquitectura EfficientNet-B0~\cite{tan2019efficientnet}.}
...
La selección de hiperparámetros para las arquitecturas CNN responde a principios establecidos en la literatura de detección de objetos con Faster R-CNN~\cite{ren_faster_2016} y a las particularidades de cada backbone. A continuación se justifican los valores empleados en cada experimento.

\paragraph{ResNet-18. Optimizador: SGD con momentum 0.9.}
SGD (Stochastic Gradient Descent) con momentum es el optimizador de referencia para Faster R-CNN con backbones ResNet desde los trabajos fundacionales de Ren et al.~\cite{ren_faster_2016}. A diferencia de los Vision Transformers, donde las capas de atención presentan gradientes de escala heterogénea que favorecen estimación adaptativa, las redes convolucionales con conexiones residuales presentan gradientes más homogéneos y SGD proporciona una regularización implícita efectiva. El momentum de 0.9 reduce la oscilación en el espacio de parámetros y acelera la convergencia en plateaus.
...
Para esta fase se seleccionó la arquitectura DEIMv2 (Dense Enhanced Image Matching), un detector en tiempo real que integra representaciones de DINOv3 con un decoder optimizado para detección~\cite{huang2025deimv2}. Huang et al. demostraron que DEIMv2 establece nuevos récords de rendimiento en COCO manteniendo eficiencia computacional; en particular, la variante DEIMv2-S fue el primer modelo con menos de 10M parámetros en superar 50 AP. Su diseño resulta especialmente adecuado para dominios industriales con variabilidad visual elevada. La arquitectura combina:
\begin{itemize}
...
Esta primera iteración con resolución 640×640 demostró que DEIMv2 supera significativamente a las CNNs (0.499 vs 0.162 de EfficientNet), pero reveló una limitación crítica: el redimensionamiento a 640×640 no es un capricho de implementación (es una necesidad técnica impuesta por restricciones de memoria), pero implica perder aproximadamente el 82\,\% de la información visual del dataset original (~1650×1350~px). Esta pérdida invalida la detección fiable de defectos milimétricos y detalles finos, justificando el incremento de resolución en iteraciones posteriores.
...
Tras los excelentes resultados de DEIMv2, surge la pregunta crítica: \textit{¿la mejora se debe a la arquitectura Vision Transformer o simplemente a usar resolución 1024×1024?}

Para responderlo científicamente, se diseñó un protocolo de validación: re-entrenar ResNet-18 y EfficientNet-B0 con las mismas condiciones que DEIMv2 (resolución, dataset, conjunto de test). Si las CNNs mejoraran significativamente con 1024px, la resolución sería el factor dominante; si no, la arquitectura ViT quedaría demostrada como superior.
...
Esta fase de validación experimental permite responder de forma definitiva a la pregunta planteada: la superioridad de DEIMv2 se debe a la arquitectura Vision Transformer, no a la resolución de entrada.
...
Tras obtener precisión perfecta (1.0) en todas las clases con el modelo DEIMv2, surge una pregunta crítica: \textit{¿dicha precisión refleja un modelo genuinamente bien entrenado o podría ser un artefacto de un umbral de confianza demasiado permisivo (0.15)?}
...
La validación con umbrales altos confirma lo siguiente:
\begin{enumerate}
    \item La precisión perfecta no es artefacto del umbral bajo.
    \item El modelo mantiene mAP$>$0.70 incluso con umbral 0.90.
    \item No hay evidencia de sobreajuste.
\end{enumerate}
El análisis detallado se presenta en el Capítulo~\ref{ch:resultados}.
